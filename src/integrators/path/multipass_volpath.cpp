 ...
 * }
 */
class MultiplePassVolumetricPathTracer : public MonteCarloIntegrator {
 public:
  MultiplePassVolumetricPathTracer(const Properties &props)
      : MonteCarloIntegrator(props) {
    if (props.hasProperty("passCount") && props.hasProperty("sppPerPass")) {
      Log(EError, "Only specify total pass count or spp per pass");
    } else if (props.hasProperty("sppPerPass")) {
      m_currentSampleCount = props.getInteger("sppPerPass");
      m_passCount = -1;
    } else {
      m_passCount = props.getInteger("passCount", 1);
    }
  }

  /// Unserialize from a binary data stream
  MultiplePassVolumetricPathTracer(Stream *stream, InstanceManager *manager)
    ...
    while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
      ...
      if (rRec.medium &&
        ...
        */
        ...
        if (!(rRec.type & RadianceQueryRecord::EIndirectMediumRadiance)) break;
        ...
        */
        if (rRec.medium) throughput *= mRec.transmittance / mRec.pdfFailure;
        ...
        if (rRec.depth >= m_maxDepth && m_maxDepth != -1) break;

        /* Prevent light leaks due to the use of shading normals */
        Float wiDotGeoN = -dot(its.geoFrame.n, ray.d),
              wiDotShN = Frame::cosTheta(its.wi);
        if (wiDotGeoN * wiDotShN < 0 && m_strictNormals) break;
        ...
        if (!(rRec.type & RadianceQueryRecord::EIndirectSurfaceRadiance)) break;
  ...
  void rayIntersectAndLookForEmitter(const Scene *scene, Sampler *sampler,
    ...
    while (true) {
      ...
      if (transmittance.isZero()) return;
  ...
  std::string toString() const override {
    ...
    oss << "MultiplePassVolumetricPathTracer[" << endl
  ...
  bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
    ...
    if (m_passCount == -1) {
      m_passCount = static_cast<int>(
          std::ceil(static_cast<Float>(sampler->getSampleCount()) /
                    static_cast<Float>(m_currentSampleCount)));
    } else {
      m_currentSampleCount = sampler->getSampleCount() / m_passCount;
    }
    Log(EInfo,
        "Starting render job (%ix%i, " SIZE_T_FMT " %s, " SIZE_T_FMT
        " %s, " SSE_STR ", pass count: %d) ..",
        film->getCropSize().x, film->getCropSize().y, sampleCount,
        sampleCount == 1 ? "sample" : "samples", nCores,
        nCores == 1 ? "core" : "cores", m_passCount);

    for (m_currentPass = 0; m_currentPass < m_passCount; ++m_currentPass) {
      if (m_currentPass == m_passCount - 1) {
        m_currentSampleCount = sampler->getSampleCount() -
                               (m_passCount - 1) * m_currentSampleCount;
      }
      Log(EInfo, "start pass %d, current sample count: " SIZE_T_FMT,
          m_currentPass, m_currentSampleCount);
      ...
      if (proc->getReturnStatus() != ParallelProcess::ESuccess) return false;
    }
  ...
  void renderBlock(
    ...
    Float diffScaleFactor = 1.0f / std::sqrt((Float)m_currentSampleCount);
    ...
    for (size_t i = 0; i < points.size(); ++i) {
      ...
      for (size_t j = 0; j < m_currentSampleCount; j++) {
...
MTS_IMPLEMENT_CLASS_S(MultiplePassVolumetricPathTracer, false,
                      MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(MultiplePassVolumetricPathTracer,
                  "Multiple Pass Volumetric path tracer");
MTS_NAMESPACE_END

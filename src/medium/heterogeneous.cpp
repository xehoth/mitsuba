...
static StatsCounter avgRayMarchingStepsTransmittance(
    ...
    EAverage);
static StatsCounter avgRayMarchingStepsSampling(
    "Heterogeneous volume", "Avg. # of ray marching steps (sampling)",
    EAverage);
...
class HeterogeneousMedium : public Medium {
 public:
  ...
  enum EIntegrationMethod {
    ...
    /**
     * \brief Use raito tracking from paper:
     * Novák J, Selle A, Jarosz W. Residual ratio
     * tracking for estimating attenuation in participating media[J]. ACM Trans.
     * Graph., 2014, 33(6): 179:1-179:11.
     * See details:
  ...
  HeterogeneousMedium(const Properties &props) : Medium(props) {
    ...
    if (props.hasProperty("densityMultiplier"))
      Log(EError,
          "The 'densityMultiplier' parameter has been deprecated and is now "
          "called 'scale'.");
  ...
  Spectrum evalTransmittance(const Ray &ray, Sampler *sampler) const {
    if (m_method == ESimpsonQuadrature || sampler == NULL) {
      ...
      for (;;) {
        t += -math::fastlog(1 - sampler->next1D()) * m_invMaxDensity;
        if (t >= maxt) break;
        // mu_n / mu_bar = (mu_bar - mu) / mu_bar = 1 - mu / mu_bar
        // = 1 - density / max_density
        Point p = ray(t);
        Float density = lookupDensity(p, ray.d) * m_scale;
